...
SELECT 
	...
	(ISNULL(A.BERK_IR_F_GRADE,0) + ISNULL(A.BERK_IR_FAIL_PF,0) + ISNULL(A.BERK_IR_WD_GRADES,0) + ISNULL(A.BERK_IR_DROP_CLASS,0)) 
		AS '# of F/W/I',
	SUM(CASE WHEN C.LOCATION <> 'ONLINE' AND C.CANCEL_DT IS NULL THEN B.UNT_TAKEN ELSE 0 END) 
		OVER(PARTITION BY A.EMPLID) AS '# ONS Credit',
	SUM(CASE WHEN C.LOCATION = 'ONLINE' AND C.CANCEL_DT IS NULL THEN B.UNT_TAKEN ELSE 0 END) 
		OVER(PARTITION BY A.EMPLID) AS '# ONL Credit',
	SUM(CASE WHEN C.LOCATION NOT LIKE '%'+A.CAMPUS+'%' AND C.CANCEL_DT IS NULL THEN B.UNT_TAKEN ELSE 0 END) 
		OVER(PARTITION BY A.EMPLID) AS '# Non-Home Credit',
	A.UNT_TAKEN_PRGRSS AS 'Total Term Credits',
	(SELECT COUNT (D.RQ_AA_WHO_DATA) FROM PS_AA_OVERRIDE D WHERE A.EMPLID = D.RQ_AA_WHO_DATA AND A.ACAD_PROG = D.ACAD_PROG 
	AND A.ACAD_PLAN = D.ACAD_PLAN AND D.EFFDT > '2018-01-01') AS '# Substitutions'
